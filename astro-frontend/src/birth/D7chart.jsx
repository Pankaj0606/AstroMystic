...
const D7chart = () => {
  ...
  useEffect(() => {
    ...
    if (astrologyData) {
      ...
      const fetchD7Chart = async () => {
        try {
          const response = await fetch("https://json.freeastrologyapi.com/d7-chart-info", {
            method: "POST",
            headers: {
          ...
          setError('Error fetching D9 chart data'); // Handle errors
        }
      };

      fetchD7Chart(); // Trigger API call
    }
  ...
  if (!chartData) {
    return <div>Loading D7 Chart...</div>; // Loading state
  }
