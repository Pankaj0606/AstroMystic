...
const BirthChart = () => {
    ...
    useEffect(() => {
        if (astrologyData) {
            const planetData0 = astrologyData.output[0]; // Planets in indexed format (e.g., 0, 1, 2...)
            const planetData1 = astrologyData.output[1]; // Planets by name (e.g., "Sun", "Moon"...)

            setAscendantSign(planetData0["0"].current_sign); // Ascendant sets the starting house

            // Use planetData1 directly as needed
            const chartData = {
                ...planetData0, // Optionally, keep indexed data
                ...planetData1  // Use named data if necessary
            };

            setChartData(chartData); // This may still be useful if you need both
            console.log(astrologyData);
        } else {
            console.log("No astrologyData");
        }
    }, [astrologyData]);
    ...
    const getPlanetsInHouse = (houseNumber) => {
        ...
        Object.entries(astrologyData.output[0]).forEach(([planetKey, details]) => {
            if (details.current_sign === adjustedHouseNumber) {
                ...
                if (!seenPlanets.has(planetName)) {  // Check if planet was already added
