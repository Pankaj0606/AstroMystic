...
const D11chart = () => {
  ...
  useEffect(() => {
    ...
    if (astrologyData) {
      ...
      const fetchD11Chart = async () => {
        try {
          const response = await fetch("https://json.freeastrologyapi.com/d11-chart-info", {
            method: "POST",
            headers: {
          ...
          setError('Error fetching D11 chart data'); // Handle errors
        }
      };

      fetchD11Chart(); // Trigger API call
    }
  ...
  if (!chartData) {
    return <div>Loading D11 Chart...</div>; // Loading state
  }
