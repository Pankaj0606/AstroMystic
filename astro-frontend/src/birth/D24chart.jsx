...
const D24chart = () => {
  ...
  useEffect(() => {
    ...
    if (astrologyData) {
      ...
      const fetchD24Chart = async () => {
        try {
          const response = await fetch("https://json.freeastrologyapi.com/d24-chart-info", {
            method: "POST",
            headers: {
          ...
          setError('Error fetching D24 chart data'); // Handle errors
        }
      };

      fetchD24Chart(); // Trigger API call
    }
  ...
  if (!chartData) {
    return <div>Loading D24 Chart...</div>; // Loading state
  }
