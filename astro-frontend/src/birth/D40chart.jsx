...
const D40chart = () => {
  ...
  useEffect(() => {
    ...
    if (astrologyData) {
      ...
      const fetchD40Chart = async () => {
        try {
          const response = await fetch("https://json.freeastrologyapi.com/d40-chart-info", {
            method: "POST",
            headers: {
          ...
          setError('Error fetching D40 chart data'); // Handle errors
        }
      };

      fetchD40Chart(); // Trigger API call
    }
  ...
  if (!chartData) {
    return <div>Loading D40 Chart...</div>; // Loading state
  }
