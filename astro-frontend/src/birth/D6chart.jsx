...
const D6chart = () => {
  ...
  useEffect(() => {
    ...
    if (astrologyData) {
      ...
      const fetchD6Chart = async () => {
        try {
          const response = await fetch("https://json.freeastrologyapi.com/d6-chart-info", {
            method: "POST",
            headers: {
  ...
  if (!chartData) {
    return <div>Loading D6 Chart...</div>; // Loading state
  }
