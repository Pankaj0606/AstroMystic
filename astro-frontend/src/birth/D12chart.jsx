...
const D12chart = () => {
  ...
  useEffect(() => {
    ...
    if (astrologyData) {
      ...
      const fetchD12Chart = async () => {
        try {
          const response = await fetch("https://json.freeastrologyapi.com/d12-chart-info", {
            method: "POST",
            headers: {
          ...
          setError('Error fetching D12 chart data'); // Handle errors
        }
      };

      fetchD12Chart(); // Trigger API call
    }
  ...
  if (!chartData) {
    return <div>Loading D12 Chart...</div>; // Loading state
  }
