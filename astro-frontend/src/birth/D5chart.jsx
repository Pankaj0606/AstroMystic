...
const D5chart = () => {
  ...
  useEffect(() => {
    ...
    if (astrologyData) {
      ...
      const fetchD5Chart = async () => {
        try {
          const response = await fetch("https://json.freeastrologyapi.com/d5-chart-info", {
            method: "POST",
            headers: {
  ...
  if (!chartData) {
    return <div>Loading D5 Chart...</div>; // Loading state
  }
