...
const DivisionalChart = ({ chartId }) => { // e.g., chartId = "D9"
    const { astrologyData } = useContext(AstrologyContext);

    if (!astrologyData || !astrologyData.chart || !astrologyData.chart.varga[chartId]) {
        return <div>Error in Loading {chartId} Chart. Try again...</div>;
    }

    const chartData = astrologyData.chart.varga[chartId];
    const ascendantSign = chartData.lagna.Lg.rashi;

    // --- Re-use the same logic as the main Birthchart.jsx ---
    const planetAbbreviations = { 
        Lg: 'Asc', Su: 'Su', Mo: 'Mo', Ma: 'Ma', Me: 'Me', 
    ...
    const getPlanetsInHouse = (astrologicalHouseNumber) => {
        ...
        if (chartData.lagna.Lg.rashi === astrologicalHouseNumber) {
            planetsInHouse.push(planetAbbreviations.Lg);
        }
        for (const planetKey in chartData.graha) {
            if (chartData.graha[planetKey].rashi === astrologicalHouseNumber) {
    ...
    // --- Paste your chart drawing functions here AGAIN ---
    // getPolygonCoordinates, calculateCentroid, arrangePlanetsInHouse, getTextPosition
        ...
        <div className="birth-chart-container">
            ...
            <svg viewBox="0 0 300 300" className="birth-chart-svg">
                ...
                {[...Array(12)].map((_, index) => {
                    const visualBoxNumber = index + 1;
                    const astrologicalHouseNumber = getHouseNumber(index);

                    const polygon = getPolygonCoordinates(visualBoxNumber);
                    const centroid = calculateCentroid(polygon);
                    const planetsInHouse = getPlanetsInHouse(astrologicalHouseNumber);
                    ...
                    return (
                        <g key={visualBoxNumber}>
                            {/* Display polygon */}
                            <polygon points={polygon} stroke="orange" fill="none" 
                            className={`house-polygon house-${astrologicalHouseNumber}`} 
                            />
                            {/* House number */}
                            ...
                            {planetsInHouse.length > 0 && planetPositions.map((pos, i) => (
                                <text key={i} x={pos.x} y={pos.y} className="planet-name">
                                    {planetsInHouse[i]}
                                </text>
                            ))}
