...
const D27chart = () => {
  ...
  useEffect(() => {
    ...
    if (astrologyData) {
      ...
      const fetchD27Chart = async () => {
        try {
          const response = await fetch("https://json.freeastrologyapi.com/d27-chart-info", {
            method: "POST",
            headers: {
          ...
          setError('Error fetching D27 chart data'); // Handle errors
        }
      };

      fetchD27Chart(); // Trigger API call
    }
  ...
  if (!chartData) {
    return <div>Loading D27 Chart...</div>; // Loading state
  }
