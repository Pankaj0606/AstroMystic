...
const D8chart = () => {
  ...
  useEffect(() => {
    ...
    if (astrologyData) {
      ...
      const fetchD8Chart = async () => {
        try {
          const response = await fetch("https://json.freeastrologyapi.com/d8-chart-info", {
            method: "POST",
            headers: {
          ...
          setError('Error fetching D8 chart data'); // Handle errors
        }
      };

      fetchD8Chart(); // Trigger API call
    }
  ...
  if (!chartData) {
    return <div>Loading D8 Chart...</div>; // Loading state
  }
