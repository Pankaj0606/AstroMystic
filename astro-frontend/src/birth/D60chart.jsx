...
const D60chart = () => {
  ...
  useEffect(() => {
    ...
    if (astrologyData) {
      ...
      const fetchD60Chart = async () => {
        try {
          const response = await fetch("https://json.freeastrologyapi.com/d60-chart-info", {
            method: "POST",
            headers: {
          ...
          setError('Error fetching D60 chart data'); // Handle errors
        }
      };

      fetchD60Chart(); // Trigger API call
    }
  ...
  if (!chartData) {
    return <div>Loading D60 Chart...</div>; // Loading state
  }
