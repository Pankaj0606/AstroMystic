...
const D4chart = () => {
  ...
  useEffect(() => {
    ...
    if (astrologyData) {
      ...
      const fetchD4Chart = async () => {
        try {
          const response = await fetch("https://json.freeastrologyapi.com/d4-chart-info", {
            method: "POST",
            headers: {
  ...
  if (!chartData) {
    return <div>Loading D4 Chart...</div>; // Loading state
  }
