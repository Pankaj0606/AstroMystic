...
const D30chart = () => {
  ...
  useEffect(() => {
    ...
    if (astrologyData) {
      ...
      const fetchD30Chart = async () => {
        try {
          const response = await fetch("https://json.freeastrologyapi.com/d30-chart-info", {
            method: "POST",
            headers: {
          ...
          setError('Error fetching D30 chart data'); // Handle errors
        }
      };

      fetchD30Chart(); // Trigger API call
    }
  ...
  if (!chartData) {
    return <div>Loading D30 Chart...</div>; // Loading state
  }
