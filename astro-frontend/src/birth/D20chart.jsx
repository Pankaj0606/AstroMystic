...
const D20chart = () => {
  ...
  useEffect(() => {
    ...
    if (astrologyData) {
      ...
      const fetchD20Chart = async () => {
        try {
          const response = await fetch("https://json.freeastrologyapi.com/d20-chart-info", {
            method: "POST",
            headers: {
          ...
          setError('Error fetching D20 chart data'); // Handle errors
        }
      };

      fetchD20Chart(); // Trigger API call
    }
  ...
  if (!chartData) {
    return <div>Loading D20 Chart...</div>; // Loading state
  }
