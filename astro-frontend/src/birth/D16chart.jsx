...
const D16chart = () => {
  ...
  useEffect(() => {
    ...
    if (astrologyData) {
      ...
      const fetchD16Chart = async () => {
        try {
          const response = await fetch("https://json.freeastrologyapi.com/d16-chart-info", {
            method: "POST",
            headers: {
          ...
          setError('Error fetching D16 chart data'); // Handle errors
        }
      };

      fetchD16Chart(); // Trigger API call
    }
  ...
  if (!chartData) {
    return <div>Loading D16 Chart...</div>; // Loading state
  }
