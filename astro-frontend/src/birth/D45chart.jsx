...
const D45chart = () => {
  ...
  useEffect(() => {
    ...
    if (astrologyData) {
      ...
      const fetchD45Chart = async () => {
        try {
          const response = await fetch("https://json.freeastrologyapi.com/d45-chart-info", {
            method: "POST",
            headers: {
          ...
          setError('Error fetching D45 chart data'); // Handle errors
        }
      };

      fetchD45Chart(); // Trigger API call
    }
  ...
  if (!chartData) {
    return <div>Loading D45 Chart...</div>; // Loading state
  }
