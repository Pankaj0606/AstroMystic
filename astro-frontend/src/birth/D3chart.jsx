...
const D3chart = () => {
  ...
  useEffect(() => {
    ...
    if (astrologyData) {
      ...
      const fetchD3Chart = async () => {
        try {
          const response = await fetch("https://json.freeastrologyapi.com/d3-chart-info", {
            method: "POST",
            headers: {
  ...
  if (!chartData) {
    return <div>Loading D3 Chart...</div>; // Loading state
  }
